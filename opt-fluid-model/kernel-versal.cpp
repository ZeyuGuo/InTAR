        ...
        compute:
                for(int k = 0; k < k_bound; k++){ // reduction dim
                    ...
                    for(int ii = 0; ii < 16; ii++){
                        #pragma HLS unroll
                        if(stage == 3){
                            op1_mtx[ii] = ap_uint<72>((ap_int<4>(2), W[k*8+ii%8][j*2+ii/8])); // dummy to enforce dotpra3 binding
                            ...
                            op1_mtx[ii] = ap_uint<72>((ap_int<4>(2), W[j*16+ii][k]));
               ...
               if(stage == 0){
                    ...
                    for(int ii = 0; ii < 16; ii++){
                        ...
                        for(int k = 0; k < 16; k++){
                            #pragma HLS unroll
                            int offset = k%8;
                            scratchpad_k[i*16+ii][j*2+k/8](offset*8+7, offset*8) = ap_int<8>(acc_final[ii][k] >> 8);
                        }
                        for(int k = 0; k < 2; k++){
                            #pragma HLS unroll
        ...
        compute:
                for(int k = 0; k < k_bound; k++){
                    ...
                    for(int ii = 0; ii < 16; ii++){
                        #pragma HLS unroll
                        if(stage == 3){
                            op1_mtx[ii] = ap_uint<72>((ap_int<4>(2), W[k*8+ii%8][j*2+ii/8]));
                            op2_mtx[ii] = ap_uint<72>((ap_int<8>(2), recv_pkt(ii*64+63, ii*64)));
                        } else if(stage != 2) {
                            op1_mtx[ii] = ap_uint<72>((ap_int<4>(2), W[j*16+ii][k]));
                            op2_mtx[ii] = ap_uint<72>((ap_int<8>(2), X[i*16+ii][k]));
                        } else {
                            op1_mtx[ii] = ap_uint<72>((ap_int<4>(2), cache_attn[k][ii]));
                ...
                if(stage == 0){
                    for(int ii = 0; ii < 16; ii++){
                        ...
                        for(int k = 0; k < 16; k++){
                            #pragma HLS unroll
                            int offset = k%8;
                            scratchpad[i*16+ii][j*2+k/8](offset*8+7, offset*8) = ap_int<8>(acc_final[k][ii] >> 8); 
                        }
                    }
                } else if (stage == 2){
                    for(int ii = 0; ii < 2; ii++){
                        ...
                        for(int jj = 0; jj < 8; jj++){
                            ...
                            for(int k = 0; k < 16; k++){
                                #pragma HLS unroll
                                tmp((jj*16+k)*8+7, (jj*16+k)*8) = ap_int<8>(acc_final[k][ii*8+jj] >> 8);
                            }
                        }
                        fifo_O_out.write(tmp);
                    }
                    ...
                    for(int ii = 0; ii < 16; ii++){
                        ...
                        for(int k = 0; k < 16; k++){
                            #pragma HLS unroll
                            tmp(k*8+7, k*8) = ap_int<8>(acc_final[ii][k] >> 8);
                        }
