...
void temporal_acc0_slr0(
    ...
    #pragma HLS array_partition variable=scratchpad_k cyclic dim=1 factor=2
            ...
            for(int kk = 1; kk < 8; kk++){
                for(int ii = 0; ii < 4; ii++){
                    ...
                    for(int k = 0; k < D_head; k++){
                        ...
                        if(kk == 7) acc_vec[ii][0][k] = std::min(std::max(acc_vec[ii][0][k] >> 8, -128), 127); // rescale & clamp
                    }
                }
            }
            ...
            if(stage == 0){
                for(int ii = 0; ii < 4; ii++){
                    ...
                    for(int k = 0; k < D_head; k++){
                        #pragma HLS unroll
                        int offset = k%8;
                        scratchpad[i*4+ii][k/8](offset*8+7, offset*8) = ap_int<8>(acc_vec[ii][0][k]);
                    }
                }
            } else {
                for(int ii = 0; ii < 4; ii++){
                    ...
                    for(int k = 0; k < D_head; k++){
                        #pragma HLS unroll
                        int offset = k%8;
                        scratchpad_k[i*4+ii][k/8](offset*8+7, offset*8) = ap_int<8>(acc_vec[ii][0][k]);
                    }
                    ...
                    for(int kk = 0; kk < 8; kk++){
                        ...
                        for(int l = 0; l < D_head_div_16; l++){
                            ...
                            for(int m = 0; m < 8; m++){ // dsp packing 8 * (4+4)
                                ...
                                ap_int<12> res0;
                                ap_int<12> res1;
                                (res1, res0) = ap_int<8>(x_vec(kk*8+7, kk*8)) * w_pack;
                                res1 = res1 + res0[11];
                                acc_vec[ii][kk][l*16+m*2] += res0;
                                acc_vec[ii][kk][l*16+m*2+1] += res1;
                            }
    ...
    reduction:
            ...
            if(stage == 0){
                ...
                for(int ii = 0; ii < 4; ii++){
                    ...
                    for(int k = 0; k < D_head; k++){
                        #pragma HLS unroll
                        tmp(k*8+7, k*8) = ap_int<8>(acc_vec[ii][0][k]);
                    }
