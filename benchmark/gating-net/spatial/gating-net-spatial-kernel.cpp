...
/**
 * @brief Multiply combined with down projection weight
 * 
...
void down_projection(
    ...
    vec_t result;
    vec_t down_col[HD_div_VEC_LEN];
    vec_t combined[B_div_VEC_LEN][HD];
    vec_t tmp_out;

#pragma HLS bind_storage variable=combined type=ram_2p impl=uram

    // readin and cache combined result
    for(int j = 0; j < HD; j++){
        for(int i = 0; i < B_div_VEC_LEN;){
            if(!combined_in_fifo.empty()){
                vec_t tmp_vec; combined_in_fifo.try_read(tmp_vec);
                combined[i][j] = tmp_vec;
                i++;
            }
        }
    }
    ...
    for (int j = 0; j < ID; j++) {  // for column j in output
// pragma HLS PIPELINE II=1

        // readin a column of down projection weight
        for (int i = 0; i < HD_div_VEC_LEN;){
            if(!down_in_fifo.empty()){
                down_in_fifo.try_read(down_col[i]);
                i++;
            }
        }
        ...
        for (int i = 0; i < B_div_VEC_LEN; i++){
            for (int ii = 0; ii < VEC_LEN; ii++){
                ...
                for(int k = 0; k < ID_div_VEC_LEN; k++){
#pragma HLS pipeline II=1
                    auto a = combined[i * VEC_LEN + ii][k];  // row i segment k of input 
                    auto b = down_col[k];  // segment k of weight
                    for (int l = 0; l < VEC_LEN; l++){
                        c += a[l] * b[l];
                    }
                }
...
void silu(
    ...
    for (int i = 0; i < silu_bound; i++){
        ...
        for (int j = 0; j < VEC_LEN; j++){
            tmp_input[j] = tmp_input[j] / (1 + exp(-tmp_input[j]));
        }
...
void gating_net(
    ...
    tapa::stream<vec_t> fifo_down("fifo_down");
    ...
    tapa::task()
        ...
        .invoke<tapa::join>(add, fifo_up_silu, fifo_gate, fifo_down)  // down = up + gate

        .invoke<tapa::join>(projection, fifo_down, fifo_W_down, fifo_output)  // output = down * W_down
