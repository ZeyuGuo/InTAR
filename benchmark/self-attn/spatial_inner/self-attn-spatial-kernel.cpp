    ...
    for(int i = 0; i < N; i++){
        for(int j = 0; j < D / VEC_LEN;){
            if(!input_in_fifo.empty()){
                ...
                for(int jj = 0; jj < VEC_LEN; jj++){
// #pragma HLS UNROLL
                    input[i][j * VEC_LEN + jj] = tmp_input[jj];
                }
                j++;
            }
        }
...
// pragma HLS PIPELINE II=1
        ...
        for (int i = 0; i < D / VEC_LEN;){
            if(!weight_in_fifo.empty()){
                ...
                for(int ii = 0; ii < VEC_LEN; ii++){
// #pragma HLS UNROLL
                    weight_col[i * VEC_LEN + ii] = tmp_weight[ii];  // NOTE: Because VEC_LEN is power of 2, muliply is faster than add
                }
                i++;
            }
        }
        ...
        for (int i = 0; i < N; i++){
            for(int k = 0; k < D; k++){
                result[i][k] += input[i][k] * weight_col[k];
            }
        }
